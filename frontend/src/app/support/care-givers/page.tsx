...
import { HeartHandshake, Loader2, Search, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function SupportCareGiversPage() {
  const { token } = useAuth();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/support/care-givers?${params}`, {
      ...
      console.error('Failed to fetch care givers:', err);
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="relative max-w-md">
              ...
              <input
                ...
                placeholder={t('admin.careGivers.searchPlaceholder')}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('admin.fields.verified')}</th>
                ...
                {isLoading ? (
                  ...
                  data.items.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center">
                            <span className="text-green-700 font-medium text-xs">{item.firstName[0]}{item.lastName[0]}</span>
                          </div>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 text-xs rounded-full ${item.isVerified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
